...
HAN_COMPILER::Compiler_CNAC compiler_CNAC;
...
void flush() {
#if debug
    {
        ...
        cout << "flush()" << " " << "\tignorecount:[" << ignorecount++ << "\t]("
                << "han_compiler_convert_non_ascii_character.cpp:" << __LINE__
                << ":0" << ")" << endl;
    }
#endif
    try {
...
std::string HAN_COMPILER::nonASCIICodeToHex(const char *nonASCIICode, unsigned int length) {
    string hexOut;
...
std::string HAN_COMPILER::nonASCIICodeToHex(const char *nonASCIICode) {
    string hexOut;
